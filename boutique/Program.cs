using System.ComponentModel;


//8) La Boutique d'équipement
//Dans un jeu, le joueur intéragi avec le marchant de la boutique pour acheter de l'equipement.
//- L'application doit permettre de gérer le stock des équipements (quantité et prix).
//- Le joueur à 250 piece pour acheter de l'équipements.

//Conditions à respecter :
//- Si l'équipement est en stock et que le joueur a l'or necessaire, l'achat est réussi.
//- Si le joueur n'a pas assez d'or pour faire l'achat, le marchant se fou de lui !
//- Si l'équipement n'est pas en stock, le marchant indique que c'est en rupture de stock.
//- Si le joueur arrive à 0 or, le marchant force le joueur à partir en ricanant.

bool gameon = true;

Console.WriteLine("Bonjour, quel est ton nom ?");
string joueur = Console.ReadLine();
int portefeuille = 250;
Console.WriteLine($"Chèr.e {joueur}, tu dispose de {portefeuille} pièces");

//Fonctionnement de l'application :
//- Créer un Dictionary<string, int> pour représenter le stock de la boutique.
Dictionary<string, int> stock = [];
Dictionary<string, int> stockj = [];


//- Initialisez le dictionnaire avec de l'équipement (ex: "Arc": 5, "Hache": 2, "Dague": 10, ...).
stock.Add("épée", 20);
stock.Add("jus de pomme", 23);
stock.Add("stiletos", 44);

while (gameon)
{
    //- Créez un menu en boucle qui propose :
    //      1.Voir le catalogue : Afficher l'équipement en stock avec son prix
    Console.WriteLine($"Stock de la boutique {String.Join("", stock)}");
    //      2. Acheter un équipement: Le joueur dépense son or pour faire un achat.
    Console.WriteLine($"Que veux-tu acheter ?");
    string request = Console.ReadLine();

        if (portefeuille == 0)
        {
        Console.WriteLine("Pauvre traîne-savattes, t'as plus un rond, Ciao !");
        gameon = false;
        }
        else if (stock.ContainsKey(request) == false)
        {
            Console.WriteLine($"nous n'avons plus de {request} en stock..");
        }
        else if (portefeuille >= stock[request])
        {
            portefeuille = portefeuille - (stock[request]);
            stockj.Add(request, stock[request]);
            stock.Remove(request);
            Console.WriteLine($"Tu as acheté {request}");
            Console.WriteLine($"Tu dispose de {portefeuille} pièces");
        }
        else if (portefeuille < stock[request])
        {
            Console.WriteLine($"Ahah. Drôle. Je ne pense pas que tu sois mon genre de clientèle.. Salut !");
        }

        //      3. Voler un équipement : Le joueur peut tenter de voler un équipement (10% de succès, en cas d'echec -> Game Over).

    Console.WriteLine($"Tentative de vol sur :");
    string vol = Console.ReadLine();

    var rand = new Random();
    int succes = rand.Next(1, 10);
        if (succes == 1)
        {
            stock.Remove(vol);
            Console.WriteLine($"Tu as réussi à voler {vol}");
            stockj.Add(vol, stock[vol]);
            Console.WriteLine($"Ton portefeuille est toujours de {portefeuille} pièces");
        }
        else
        {
            Console.WriteLine($"Tu n'as pas réussi à me voler {vol}, je t'ai vu ! Sors de la boutique !");
            gameon = false;
        }

        //      4. Quitter la boutique : Fin de l'intéraction avec le marchant. Afficher la liste des achats de joueur.
        
}// while(gameon)

if (!gameon)
{
    Console.WriteLine($"Fin du jeu, mais garde la pêche, ton stock est de {String.Join("", stockj)}");
}

